 ...
 */
public abstract class BindablePropertyBase<T>(
    public val distinct: Boolean,
    public val afterSet: AfterSet<T>?,
    public val beforeSet: BeforeSet<T>?,
    public val validate: Validate<T>?
) {
    /**
     * Base delegate provider for [BindablePropertyBase].
     */
    public abstract class Provider<VM : ViewModel, T> : PropertyDelegateProvider<VM, BindablePropertyBase<T>> {
 ...
 */
public fun <T, P : BindablePropertyBase.Provider<*, T>> P.distinct(): P = apply { internalDistinct() }

/**
 * Sets [BindablePropertyBase.beforeSet] to a given function and returns that instance.
 */
public fun <T, P : BindablePropertyBase.Provider<*, T>> P.beforeSet(action: BeforeSet<T>): P = apply { internalBeforeSet(action) }

/**
 * Sets [BindablePropertyBase.validate] to a given function and returns that instance.
 */
public fun <T, P : BindablePropertyBase.Provider<*, T>> P.validate(action: Validate<T>): P = apply { internalValidate(action) }

/**
 * Sets [BindablePropertyBase.afterSet] to a given function and returns that instance.
 */
public fun <T, P : BindablePropertyBase.Provider<*, T>> P.afterSet(action: AfterSet<T>): P = apply { internalAfterSet(action) }
