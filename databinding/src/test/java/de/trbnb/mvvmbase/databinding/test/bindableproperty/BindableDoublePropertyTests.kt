...
import de.trbnb.mvvmbase.databinding.bindableproperty.bindableDouble
...
class BindableDoublePropertyTests {
    @BeforeEach
    fun setup() {
    ...
    fun `is zero the default value without explicit assignment`() {
        val viewModel = object : BaseViewModel() {
            val property by bindableDouble()
        }

        assert(viewModel.property == 0.0)
    }
    ...
    fun `is validate() called`() {
        ...
        val viewModel = object : BaseViewModel() {
            var property by bindableDouble(defaultValue = oldValue)
                .validate { old, new ->
                    assert(old == oldValue)
                    assert(new == newValue)
    ...
    fun `automatic StateSaveOption is set correctly`() {
        ...
        assert(savedStateHandle.get<Double>("supportedAutomatic") == newValue)
    }
    ...
    fun `manual StateSaveOption is working correctly`() {
        ...
        val newValue = 9.0
        viewModel.property = newValue
        assert(savedStateHandle.get<Double>(key) == newValue)
    }
